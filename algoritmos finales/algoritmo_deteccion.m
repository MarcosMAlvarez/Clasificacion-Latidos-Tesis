...
%Frecuencia de muestreo utilizada por el ECG
Fs=360;
 
 
%Agrega ruido gaussiano a la señal con una 
%relación señal/ruido (RSR) determinada 
RSR=2; 
amplitud=(abs(max(x))+abs(min(x)))/2;
ruido=(amplitud/RSR)*rand(size(x));
%x=x+ruido;
 
%definen variables para pasos posteriores
clear Raa && Racp && Ranp && RRpromedio && cont && ubicacion
...
%la frecuencia de cutoff es de aprox. 11 Hz y la ganancia 49. 
%retrasa 6 muestras
...
%filtro pasa-alto
%la frecuencia de cutoff es de aprox. 5 Hz y retrasa 24 muestras
...
%guardamos la ubicacion de los latidos detectados 
dlmwrite('ubicacion.txt',[ubicacion' cont'],'delimiter', '\t');

%seleccionamos las muestras para ICA y corremos el programa FastICA
xica=lat_f(10:100,:);
